...
abstract class Vector<S : Size, T : Number, V : Vector<S, T, V>>(
    ...
    abstract fun instanciate(values: Array<T>): V
    ...
    @Suppress("UNCHECKED_CAST")
    private fun map(fn: (it: T, index: Int) -> T) = instanciate(
        components.mapIndexed { i, a -> fn(a, i) }.array() as Array<T>
    )
...
abstract class Vector2<T : Number, V : Vector2<T, V>>(numClass: KClass<T>, vecClass: KClass<V>, x: T, y: T) :
    ...
    @Suppress("UNCHECKED_CAST")
    fun perpendicular() = instanciate(arrayOf<Any>(0.to(numClass).numMinus(y), x) as Array<T>)
}
...
class Vector2f(x: Float, y: Float) : Vector2<Float, Vector2f>(Float::class, Vector2f::class, x, y) {
    ...
    override fun instanciate(values: Array<Float>) = Vector2f(values[0], values[1])
}
...
class Vector2i(x: Int, y: Int) : Vector2<Int, Vector2i>(Int::class, Vector2i::class, x, y) {
    ...
    override fun instanciate(values: Array<Int>) = Vector2i(values[0], values[1])
}
...
class Vector3f(x: Float, y: Float, z: Float) : Vector3<Float, Vector3f>(Float::class, Vector3f::class, x, y, z) {
    ...
    override fun instanciate(values: Array<Float>) = Vector3f(values[0], values[1], values[2])
}
...
class Vector4f(x: Float, y: Float, z: Float, w: Float) :
    ...
    override fun instanciate(values: Array<Float>) = Vector4f(values[0], values[1], values[2], values[3])
}
...
class Color(r: Float, g: Float, b: Float, a: Float = 1f) :
    ...
    override fun instanciate(values: Array<Float>) = Color(values[0], values[1], values[2], values[3])
}
...
class Rect(x1: Float, y1: Float, x2: Float, y2: Float) :
    ...
    override fun instanciate(values: Array<Float>) = Rect(values[0], values[1], values[2], values[3])
...
class Quaternion(a: Float, b: Float, c: Float, d: Float) :
    ...
    override fun instanciate(values: Array<Float>) = Quaternion(values[0], values[1], values[2], values[3])
